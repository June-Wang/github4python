...
def color4output(date_today,price_info_list,sh_info_list,color,persent,persent_sh):
	...
	p_change_avg = {}
	p_change_avg_day_list = [5,10,20,30,60,90,120] #增长率周期取样列表（天）
	for day in p_change_avg_day_list:
		p_change_avg[day] = day_data[day]/day

	p_change_day_msg = '/'.join(str(day) for day in p_change_avg_day_list)
	p_change_avg_msg = '\t'.join(get_color("%.2f" % p_change_avg[day]) for day in p_change_avg_day_list)
	#p_change_msg = '\tAVG('+p_change_day_msg+')\t'+p_change_avg_msg
	p_change_msg = '\t|\t'+p_change_avg_msg
