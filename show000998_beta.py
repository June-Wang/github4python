...
if __name__ == "__main__":
    ...
    for day in day_list:
        ...
        p_msg = "P(1/"+'/'.join(str(i) for i in day_list_persent)+")\t"+get_color(("%.2f" % float(price_wave)))+'\t'+cycle_p_change_msg+'\t'+low_persent_msg
        ...
        persent90 = float(persent_cycle[90][day])
        persent30_90_avg = (persent30+persent60+persent90)/3

        #if (p10 <= -15 and persent30 <= -90) or (avg_persent <= -90):
        #if p5 <= -10 and p10 <= -10 and persent30 <= -90 and sh_avg_persent >= -85:
        if (p10 <= -10 and avg_persent == -100 and sh_avg_persent >= -85) or (persent90 < persent60 < persent30 and persent30_90_avg <= -80):
            f1_msg = Fore.CYAN+front_msg+Style.RESET_ALL
        #elif avg_persent == 100:
        #elif p10 >= 9 and persent30 == 100 and persent60 == 100:
        elif avg_persent == 100 and sh_avg_persent >= 90:
