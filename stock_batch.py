...
def do_it(code):
    ...
    for day in day_list:
        ...
        #current_time = date.datetime.strftime('%s%N')
        shell_cmd = 'date -d '+'"'+date+'"'+' +"%s%N"'
        current_time = os.popen(shell_cmd).read()
        current_time = current_time.replace("\n", "")
        print('stock_cron,'+'code='+stock_code+' '+p_msg+w_msg+'AVG='+("%.2f" % avg_persent)+','+'SH_AVG='+("%.2f" % sh_avg_persent)+','+'LOW='+("%.2f" % low_persent)+' '+current_time)
